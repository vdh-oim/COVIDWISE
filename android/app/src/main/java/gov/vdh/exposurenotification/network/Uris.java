        ...
        .transform(
            indexContent -> {
              ...
              for (String indexEntry : indexEntries) {
                if (indexEntry.startsWith("exposureKeyExport")){
...
=                completer.setCancelled();
  ...
  void saveLastIndexToSharedPrefernce(String nameOfLastFileInIndex){
    SharedPreferences sharedPref = context.getSharedPreferences("com.springml.pref.LAST_INDEX", Context.MODE_PRIVATE);
    SharedPreferences.Editor editor = sharedPref.edit();
    editor.putString("PREVIOUS_LAST_INDEX", nameOfLastFileInIndex);// for actual
    editor.commit();
  }

  String getLastIndexFromSharedPrefernce(){
    SharedPreferences sharedPref = context.getSharedPreferences("com.springml.pref.LAST_INDEX", Context.MODE_PRIVATE);
    String lastFileName = sharedPref.getString("PREVIOUS_LAST_INDEX", "NONE");
    return lastFileName;
  }

  private List<String> getDeltaListForDownload(List<String> indexEntries, String lastIndexFromSharedPrefernce) {
    List<String> deltaList = new ArrayList<String>();
    int lastDownloadIndex = indexEntries.indexOf(lastIndexFromSharedPrefernce);
    for (;lastDownloadIndex < indexEntries.size()-1; lastDownloadIndex++){
      deltaList.add(indexEntries.get(lastDownloadIndex + 1));
    }
    if (deltaList.isEmpty()){
      deltaList = indexEntries;
    }
    return deltaList;
