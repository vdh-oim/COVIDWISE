...
import gov.vdh.exposurenotification.network.DiagnosisKey;
import gov.vdh.exposurenotification.network.DiagnosisKeys;
import gov.vdh.exposurenotification.storage.PositiveDiagnosisEntity;
import gov.vdh.exposurenotification.storage.PositiveDiagnosisRepository;
...
//    }
   ...
   * Share the keys.
   */
  public void share(ProgressDialog prog) {
    prog.show();
   ...
   */
  public void save(boolean shared,ProgressDialog prog) {
    Futures.addCallback(
  ...
  private ListenableFuture<Void> insertOrUpdateDiagnosis(boolean shared) {
    ...
    if (positiveDiagnosisId == NO_EXISTING_ID) {
      ...
      if(!shared) {
        // Not storing if it is not shared
        return repository.deleteByIdAsync(positiveDiagnosisId);
      }
      if (testTimestampLiveData.getValue() == null){
        return repository.upsertAsync(
            PositiveDiagnosisEntity.create(ZonedDateTime.now(), shared));
      }
      else {
        return repository.upsertAsync(
