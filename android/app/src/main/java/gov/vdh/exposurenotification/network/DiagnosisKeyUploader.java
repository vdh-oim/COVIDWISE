...
public class DiagnosisKeyUploader {
  ...
  private static final int FAKE_ATTESTATION_LENGTH = 5394; // Measured from a real payload
  ...
  private ListenableFuture<?> doUpload(
    ...
    return FluentFuture.from(countries)
        ...
        // internally to DiagnosisAttestor, so getLightweightExecutor() is fine here.
  ...
  private ListenableFuture<KeySubmission> addPayload(KeySubmission submission) {
    String pinToken = new ExposureNotificationSharedPreferences(context.getApplicationContext()).getPinToken("");
            ...
            attestation -> {
              ...
              String deviceVerificationPayload =
                  submission.isFakeTraffic
                      ? randomBase64Data(FAKE_ATTESTATION_LENGTH)
                      : attestation.token();
              submission.payload =
                  new JSONObject()
          ...
          CallbackToFutureAdapter.getFuture(
              completer -> {
                ...
                ErrorListener errorListener =
                    err -> {
                      CustomUtility.customLogger(String.format("A_CW_ERROR Diagnosis Key upload error: [%s]",
