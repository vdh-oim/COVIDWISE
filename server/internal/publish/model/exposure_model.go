...
import (
	...
	// Added to verify the JWE token using public key
...
func ApplyTransmissionRiskOverrides(p *verifyapi.Publish, overrides verifyapi.TransmissionRiskVector) {
	...
	fmt.Println("Printing the JWT token received in the request : ", p.JWTToken)
	// // Newly added verification for the jwt
	if len(p.JWTToken) == 0 || strings.Contains(p.JWTToken, "Error Generating Token") || len(strings.Split(p.JWTToken, ".")) != 3 {
		fmt.Println("pinToken is either empty or contains invalid string")
		return
	}

	_, err := verifyJWTToken(context.Background(), p.JWTToken)
	if err != nil && err.Error() != "token contains an invalid number of segments" {
		fmt.Println(" Found issue in JWT payload : ", err)
	}
}

// PHAClaims : This object is coming as part of JWT token
type PHAClaims struct {
	pin int64
	jwt.StandardClaims
}

func verifyJWTToken(ctx context.Context, jwtToken string) (string, error) {
	var claims *PHAClaims
	var token1 string

	var pubCertByte []byte

	// Spliting the JWT Token
	segments := strings.Split(jwtToken, ".")

	// Unpack JWT so we can determine issuer and key version.
	token, err := jwt.ParseWithClaims(jwtToken, &PHAClaims{}, func(token *jwt.Token) (interface{}, error) {
		if method, ok := token.Method.(*jwt.SigningMethodRSA); !ok || method.Name != jwt.SigningMethodRS256.Name {
			return nil, fmt.Errorf("unsupported signing method, must be RSA, found : %v", method.Name)
		}

		// Getting secret manager
		client, err := secretmanager.NewClient(ctx)
		if err != nil {
			return nil, fmt.Errorf("Unable to get the secret Client : %v", err.Error())
		}
		// Build the request to access Secret Manager key
		secretName := os.Getenv("VDH_PUBLIC_CERT")
		if secretName == "" {
			secretName = //pass in the google secret name
		}
		req := &secretmanagerpb.AccessSecretVersionRequest{
			Name: secretName,
		}

		// Call the API to access the secret manager
		result, err := client.AccessSecretVersion(ctx, req)
		if err != nil {
			return nil, fmt.Errorf("Error accessing the secret manager : %v", err.Error())
		}

		responseStr := result.Payload.Data

		block, _ := pem.Decode([]byte(responseStr))
		if block == nil {
			return nil, fmt.Errorf("Failed to parse public certificate")
		}

		pubCertByte = responseStr

		pubCert, err := x509.ParseCertificate(block.Bytes)
		if err != nil {
			return nil, fmt.Errorf("Failed to parse public certificate : %v" + err.Error())
		}

		pubKey, ok1 := pubCert.PublicKey.(*rsa.PublicKey)
		if !ok1 {
			return nil, fmt.Errorf("Public key is not retrieved")
		}

		messageBody := strings.Join(segments[:2], ".")
		publicCertJWTStr := segments[2]
		signature, err := base64.RawURLEncoding.DecodeString(publicCertJWTStr)
		if err != nil {
			return nil, fmt.Errorf("Error decoding jwt signature: %v", err.Error())
		}

		// Verify the RSA signature
		digest := sha256.Sum256([]byte(messageBody))
		...
		var ok bool
		claims, ok = token.Claims.(*PHAClaims)
		if !ok {
	...
	expireIn := claims.ExpiresAt
	if (currentTime - issuedAt) < 0 {
		return token1, fmt.Errorf("Token is issued in future : %v", claims.IssuedAt)
	}

	if (currentTime - expireIn) > 0 {
		return token1, fmt.Errorf("Token is already expired : %v", claims.ExpiresAt)
	}

	var vdhURL string
	var vdhCode string

	vdhURL = os.Getenv("VDH_URL")
	if vdhURL == "" {
		//Once verification is done, send a request to vdh server to let them know that the pin has been consumed by exposure server
		vdhURL = // pin verification URL
	}

	vdhCode = os.Getenv("VDH_CODE")
	if vdhCode == "" {
		vdhCode = // pin verification code
	}

	// Reading the standard payload and parsing pin value
	standardPayload, err := base64.StdEncoding.DecodeString(segments[1])
	vdhPayloads := strings.Split(string(standardPayload), ",")
	pinValue := strings.Trim(strings.Split(vdhPayloads[len(vdhPayloads)-1], ":")[1], "\"")

	payload := (`<s12:Envelope xmlns:s12='http://www.w3.org/2003/05/soap-envelope'>
	<s12:Body>
	  <ns0:VerificationRequest xmlns:ns0='ContactTracingTestVerificationWS'>
		<ns0:Pin>` + string(pinValue) + `</ns0:Pin>
		<ns0:Code>` + vdhCode + `</ns0:Code>
		<ns0:VerificationType>F</ns0:VerificationType>
	  </ns0:VerificationRequest>
	</s12:Body>
  </s12:Envelope>`)

	// Loading public certificate
	certPool := x509.NewCertPool()
	ok := certPool.AppendCertsFromPEM(pubCertByte)
	if !ok {
		return token1, fmt.Errorf("unable to append pubilc certificate to certificate pool")
	}

	// Getting a HTTPS client
	client := &http.Client{
	...
	req, err := http.NewRequest(http.MethodPost, vdhURL, bytes.NewBuffer([]byte(payload)))
	if err != nil {
		return token1, fmt.Errorf("Error creating HTTP Request : %v", err.Error())
	}
	// Adding header information to the request
	req.Header.Add("Content-Type", "application/soap+xml;charset=UTF-8;action='urn:VerificationRequest'")

	// Sending the request
	resp, err := client.Do(req)
	if err != nil {
		return token1, fmt.Errorf("Got error response from vdh server : %v", err.Error())
	}
	...
	if resp.StatusCode != http.StatusOK {
		return token1, fmt.Errorf("Unable to set the VerificationType on verification server : %v", err.Error())
	} else {
		// Reading the response
		body, err := ioutil.ReadAll(resp.Body)
		if err != nil {
			return token1, fmt.Errorf("Got the Error while reading the response : %v", err.Error())
		}

		if !strings.Contains(string(body), "response>YES") {
			// return http 400 to android client
			return token1, fmt.Errorf("Pin is already used")
		}
		// return http 200 to android client
		fmt.Println("Successfully set VerificationType to F on the verification server!!")
...
// KeyTransform : Added the comment for linting
...
func (t *Transformer) TransformPublish(ctx context.Context, inData *verifyapi.Publish, batchTime time.Time) ([]*Exposure, error) {
	...
	// Checking if pinToken is coming empty
	if len(inData.JWTToken) == 0 || strings.Contains(inData.JWTToken, "Error Generating the token") || len(strings.Split(inData.JWTToken, ".")) != 3 {
		return nil, fmt.Errorf("pinToken is either empty or contains invalid string")
	}
	// Newly added verification for the jwt
	_, err := verifyJWTToken(ctx, inData.JWTToken)
	if err != nil && err.Error() != "token contains an invalid number of segments" {
		return nil, fmt.Errorf(" Found issue in JWT payload : %v", err.Error())
	}
	return entities, nil
}
