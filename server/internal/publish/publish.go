...
// Error strings for JWT validateion
var (
	tokenExpired = "token is expired by"
	tokenEmpty   = "pinToken is either empty or contains invalid string"

	certificatePoolIssue = "unable to append pubilc certificate to certificate pool"

	createHTTPRequestIssue = "Error creating HTTP Request"
	responseError          = "Got error response from vdh server"
	readingResponseError   = "Got the Error while reading the response"

	setVerificationTypeError = "Unable to set the VerificationType on verification server"

	pinUsedError = "Pin is already used"
)
...
func (h *publishHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if response.status == http.StatusBadRequest {
		...
		if strings.Contains(messages, tokenExpired) || strings.Contains(messages, tokenEmpty) || strings.Contains(messages, certificatePoolIssue) || strings.Contains(messages, createHTTPRequestIssue) || strings.Contains(messages, responseError) || strings.Contains(messages, readingResponseError) || strings.Contains(messages, setVerificationTypeError) || strings.Contains(messages, pinUsedError) {
			w.WriteHeader(response.status)
		}
	}
